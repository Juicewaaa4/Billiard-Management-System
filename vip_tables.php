...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    ...
    if ($action === 'add_ktv_table') {
      ...
      $tableNumber = trim((string) ($_POST['table_number'] ?? ''));
      if ($tableNumber === '')
        throw new RuntimeException('KTV Room name is required.');
    ...
    if ($action === 'start_game') {
      ...
      $table = $stmt->fetch();
      if (!$table)
        throw new RuntimeException('VIP/KTV Room not found.');
      ...
      if ($resId > 0 && is_array($rvRow)) {
         ...
         // If reservation was for today and is in the past, use its exact time
...
<div class="row" style="gap:14px; align-items:flex-start; flex-wrap:wrap; margin-bottom:14px;">
  <?php if ((current_user()['role'] ?? '') === 'admin'): ?>
    ...
    <div class="card" style="flex:1; min-width:300px;">
      <div class="row">
        <div>
          <div class="card__title" style="color:#c084fc;">Add KTV Room</div>
        </div>
        <div class="spacer"></div>
      </div>
      <form method="post" class="form" style="margin-top:12px;">
...
<div class="row" style="gap:14px; align-items:flex-start; flex-wrap:wrap;">
    ...
    <div class="card">
      ...
      <div style="margin-top:14px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
        <div style="position:relative; flex:1; min-width:220px;">
          <input type="text" id="tableSearch" placeholder="Search VIP table name..."
      ...
      <?php foreach ($tables as $t): ?>
        ...
        <div class="card table-card" data-table-name="<?php echo h(strtolower($t['table_number'])); ?>"
          ...
          <?php if ($active): ?>
            <?php if ($hasRes): ?>
              <div
                style="padding:6px 10px; margin-bottom:10px; background:linear-gradient(90deg, rgba(56,189,248,0.08), rgba(56,189,248,0.02)); border-radius:6px; border:1px solid rgba(56,189,248,0.2);">
            ...
            <div style="background:var(--surface2); border-radius:8px; padding:12px; margin-bottom:12px;">
              ...
              $customerName = $customerId !== null ? ($customerNameById[$customerId] ?? ('Customer #' . $customerId)) : (!empty($active['walk_in_name']) ? $active['walk_in_name'] : 'Walk-in');
              ?>
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                ...
                <?php if (!empty($active['scheduled_end_time'])): ?>
                  <span class="badge badge--warn" data-countdown="<?php echo h($active['scheduled_end_time']); ?>"
                    data-session-id="<?php echo $active['id']; ?>" data-table-name="<?php echo h($t['table_number']); ?>"
            ...
            <div style="display:flex; gap:8px;">
              ...
              <button class="btn btn--ok" type="button"
                style="flex:1; background: #fdba74; border-color: #fdba74; color: #fff;"
                onclick="openLoyaltyModal(<?php echo (int) $active['id']; ?>, '<?php echo h($t['table_number']); ?>')">+Loyalty</button>
              <button class="btn btn--ghost" type="button" style="flex:1;"
                onclick="openEndModal(<?php echo (int) $active['id']; ?>, '<?php echo h($t['table_number']); ?>')">End
                Game</button>
            </div>
          <?php else: ?>
            <?php if ($hasRes): ?>
            ...
            <div style="text-align:center; padding:<?php echo $hasRes ? '8px' : '16px'; ?> 0 8px;">
              ...
              <?php if ($hasRes && $maxH <= 0 && $resInfo['reservation_date'] === date('Y-m-d')): ?>
                <a class="btn btn--primary" style="width:100%; text-decoration:none; text-align:center;"
                  href="vip_tables.php?start_reservation=<?php echo (int) $resInfo['id']; ?>">▶️ Start Reservation</a>
  ...
  <div style="flex:1; min-width:320px; display:flex; flex-direction:column; gap:14px;">
    <div class="card">
      ...
      <div style="margin-top:14px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
        <div style="position:relative; flex:1; min-width:220px;">
          <input type="text" id="ktvSearch" placeholder="Search KTV room name..."
            style="width:100%; padding-left:36px;">
          <span
            style="position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--muted); pointer-events:none; font-size:15px;">🔍</span>
        </div>
        <div style="display:flex; gap:6px;">
          <button type="button" class="btn ktv-filter-btn is-active btn--primary"
            style="background:#a855f7; border-color:#a855f7;" data-filter="all">All</button>
    ...
    <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(310px, 1fr)); gap:14px; margin-top:14px;"
      ...
      <?php foreach ($tables_ktv as $t): ?>
        ...
        <div class="card table-card" data-table-name="<?php echo h(strtolower($t['table_number'])); ?>"
          ...
          <?php if (!$isAvailable): ?>
            <div
              style="position:absolute; top:0; right:0; width:80px; height:80px; background:radial-gradient(circle at top right, rgba(168,85,247,0.1), transparent 70%); pointer-events:none;">
            </div>
          <?php endif; ?>
          ...
          <div style="display:flex; align-items:center; gap:12px; margin-bottom:12px;">
            <div style="font-size:18px; font-weight:700; color:var(--text);">
              <?php echo h($t['table_number']); ?>
              <span class="badge"
                style="margin-left:6px; font-size:11px; background:rgba(168, 85, 247, 0.2); color:#c084fc;">KTV</span>
            </div>
            <div class="spacer"></div>
            <?php if ($isAvailable): ?>
              <span class="badge badge--ok">Available</span>
          ...
          <?php if ($active): ?>
              ...
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                ...
                <?php if (!empty($active['scheduled_end_time'])): ?>
                  <span class="badge badge--warn" data-countdown="<?php echo h($active['scheduled_end_time']); ?>"
                    ...
                    data-player-name="<?php echo h($customerName); ?>"
                    data-rate="<?php echo (float) $active['rate_per_hour']; ?>"
                    style="font-size:14px; font-weight:700; background:rgba(168, 85, 247, 0.15); color:#d8b4fe;">--:--:--</span>
                <?php else: ?>
                  <span class="badge" data-timer-start="<?php echo h($active['start_time']); ?>">00:00:00</span>
                <?php endif; ?>
              </div>
            ...
            <div style="display:flex; gap:8px;">
              <button class="btn" style="background:#c084fc; color:#fff; border:none; flex:1;" type="button"
                onclick="openExtendModal(<?php echo (int) $active['id']; ?>, '<?php echo h($t['table_number']); ?>', <?php echo (float) $active['rate_per_hour']; ?>, '<?php echo h($active['scheduled_end_time'] ?? ''); ?>', <?php echo $maxExtH; ?>)">Extend</button>
              <button class="btn btn--ghost" type="button" style="flex:1;"
                onclick="openEndModal(<?php echo (int) $active['id']; ?>, '<?php echo h($t['table_number']); ?>')">End
                Session</button>
          ...
          <?php else: ?>
            ...
            <div style="text-align:center; padding:<?php echo $hasRes ? '8px' : '16px'; ?> 0 8px;">
              ...
              <?php if ($hasRes && $maxH <= 0 && $resInfo['reservation_date'] === date('Y-m-d')): ?>
                <a class="btn"
                  style="background:#a855f7; color:white; border:none; width:100%; text-decoration:none; text-align:center;"
                  href="vip_tables.php?start_reservation=<?php echo (int) $resInfo['id']; ?>">▶️ Start Reservation</a>
              <?php elseif ($hasRes && $maxH > 0 && $resInfo['reservation_date'] === date('Y-m-d')): ?>
                <div style="color:#f59e0b; font-size:11px; margin-bottom:6px; font-weight:600;">
                  ⚠️ Max <?php echo $maxH; ?>hr allowed (reserved today at <?php echo $resTimeFmt; ?>)
                </div>
                <button class="btn" type="button" style="background:#a855f7; color:white; border:none; width:100%;"
                  onclick="openStartModal(<?php echo $tid; ?>, '<?php echo h($t['table_number']); ?>', <?php echo (float) $t['rate_per_hour']; ?>, <?php echo $maxH; ?>)">Start Session</button>
              <?php elseif ($hasRes && $resInfo['reservation_date'] !== date('Y-m-d')): ?>
                <!-- Future reservation - no restriction on walk-in hours today -->
                <div style="color:#38bdf8; font-size:11px; margin-bottom:6px; font-weight:600;">
                  ℹ️ Reserved on <?php echo $resDateLabel; ?> at <?php echo $resTimeFmt; ?>
                ...
                <button class="btn" type="button" style="background:#a855f7; color:white; border:none; width:100%;"
                  onclick="openStartModal(<?php echo $tid; ?>, '<?php echo h($t['table_number']); ?>', <?php echo (float) $t['rate_per_hour']; ?>, 0)">Start Session</button>
              <?php else: ?>
                <button class="btn" type="button" style="background:#a855f7; color:white; border:none; width:100%;"
                  onclick="openStartModal(<?php echo $tid; ?>, '<?php echo h($t['table_number']); ?>', <?php echo (float) $t['rate_per_hour']; ?>, 0)">Start Session</button>
              <?php endif; ?>
            ...
            <details style="margin-top:10px; border-top:1px solid var(--border); padding-top:10px;">
              ...
              <div style="margin-top:10px;">
                <form method="post" class="form" style="margin-bottom:10px;">
                  ...
                  <button class="btn btn--block" style="background:#a855f7; color:white; border:none; font-size:12px;"
    ...
    <div class="game-modal__body">
      ...
      <div id="karaokeOptionWrapper">
        ...
        <div style="display:flex; gap:10px; margin-bottom:14px;" id="karaokeOption">
          <label
            style="flex:1; border:1px solid var(--border); padding:10px; border-radius:8px; display:flex; align-items:center; justify-content:center; gap:8px; cursor:pointer; background:var(--surface2);">
            <input type="radio" name="temp_karaoke" value="0" checked
              onchange="document.getElementById('sf_karaoke').value='0'">
            <span style="font-size:13px; font-weight:600;">Without Karaoke</span>
          </label>
          <label
            style="flex:1; border:1px solid var(--border); padding:10px; border-radius:8px; display:flex; align-items:center; justify-content:center; gap:8px; cursor:pointer; background:var(--surface2);">
            <input type="radio" name="temp_karaoke" value="1"
              onchange="document.getElementById('sf_karaoke').value='1'">
  ...
  <div class="game-modal__box" style="max-width:400px;">
    ...
    <div class="game-modal__body" style="text-align:center; padding:28px 24px;">
      <div
        ...
        👑</div>
      <p style="color:var(--text); font-size:15px; margin:0 0 8px;">Are you sure you want to <strong>end this VIP
    ...
    <div class="game-modal__body" style="padding:28px 24px;">
      <div style="text-align:center; margin-bottom:16px;">
        <div
          style="width:56px; height:56px; margin:0 auto 16px; border-radius:50%; background:rgba(239,68,68,0.12); display:flex; align-items:center; justify-content:center; font-size:28px;">
          ❌</div>
        <p style="color:var(--text); font-size:15px; margin:0 0 8px;">Are you sure you want to <strong>void this VIP
            game</strong>?</p>
  ...
  <div class="game-modal__box" style="max-width:400px;">
    ...
    <div class="game-modal__body" style="padding:28px 24px; text-align:center;">
      <div
        ...
        🎁</div>
      <p style="color:var(--text); font-size:15px; margin:0 0 8px;">Are you sure you want to add <strong>1 Free
          Hour</strong> of loyalty time to <strong id="loyaltyTableName"></strong>?</p>
      <p style="color:var(--muted); font-size:13px; margin:0;">This will not be charged and will reflect as a loyalty
        reward in exports.</p>
      <div class="game-modal__footer" style="justify-content:center; margin-top:24px;">
        <button type="button" class="btn btn--ghost" onclick="closeLoyaltyModal()">Cancel</button>
        <button type="button" class="btn" style="background:#fdba74; color:#fff; border:none;"
          onclick="submitLoyalty()">Confirm +Loyalty</button>
      </div>
    </div>
  </div>
</div>
...
<style>
  ...
  .game-modal__summary {
    background: var(--bg-secondary, var(--surface2));
    padding: 14px;
    border-radius: 8px;
    margin-top: 14px;
  }
...
<script>
  ...
  <?php if (!empty($prefillReservation)): ?>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Explicitly enforce karaoke option visibility based on table type
      // (openStartModal handles this, but ensure it's correct after prefill runs)
      const karaokeWrapper = document.getElementById('karaokeOptionWrapper');
      if (karaokeWrapper) {
        if (isKtv) {
          karaokeWrapper.style.display = 'none';
          document.getElementById('sf_karaoke').value = '1';
        } else {
          karaokeWrapper.style.display = 'block';
          document.getElementById('sf_karaoke').value = '0';
          const noKaraokeRadio = document.querySelector('input[name="temp_karaoke"][value="0"]');
          if (noKaraokeRadio) noKaraokeRadio.checked = true;
        }
      }
...
<div id="dupModal" class="game-modal" style="display:none; z-index: 100000;"
  onclick="if(event.target.id==='dupModal')closeDupModal()">
  <div class="game-modal__box" style="max-width:400px;">
    <div class="game-modal__header">
